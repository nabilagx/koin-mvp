...
export default async function ParentDashboardPage({
  ...
  searchParams: Promise<{ child_id?: string; error?: string; success?: string }>;
}) {
    ...
    <AppShell user={user} title="Parent Dashboard">
      <DashboardNav
        links={[
          ...
          { href: "#saldo-limit", label: "Saldo & Limit" },
          { href: "#kartu", label: "Kartu" },
          { href: "#celengan", label: "Celengan" },
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
        <section id="tambah-anak" className="panel rounded-lg p-5">
        ...
        {child ? (
          <section className="grid gap-6">
            ...
            <section id="saldo-limit" className="panel rounded-lg p-5">
              <h2 className="text-lg font-black">Saldo, Limit, dan PIN</h2>
              <div className="mt-4 grid gap-3 md:grid-cols-2">
                <form action={topUpChildAction} className="flex gap-2">
                  ...
                  <SubmitButton className="btn-primary" pendingText="Top up...">Top up</SubmitButton>
                </form>
                ...
                <form action={updateChildPinAction} className="flex gap-2 md:col-span-2">
                  <input type="hidden" name="child_id" value={child.id} />
            ...
            <section id="kartu" className="panel rounded-lg p-5">
              <h2 className="text-lg font-black">Kartu</h2>
              <p className="mt-2 text-sm text-ink/65">UID: {child.card_uid ?? "Belum ada kartu"} - Status: {child.card_status ?? "-"}</p>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-2">
          <section className="panel rounded-lg p-5">
            ...
            <div className="mt-3 divide-y divide-line">
              {data.transactions.map((item) => <div className="py-3 text-sm" key={item.id}>{item.status} - {formatRupiah(Number(item.amount))} - {item.failure_reason ?? "success"} - {formatDateTime(item.created_at)}</div>)}
              {data.transactions.length === 0 ? <EmptyState title="Belum ada transaksi" description="Transaksi anak terpilih akan muncul di sini." /> : null}
            </div>
          </section>

          <section className="panel rounded-lg p-5">
            <h2 className="text-lg font-black">Riwayat Top-up</h2>
            <div className="mt-3 divide-y divide-line">
              ...
              {data.topups.length === 0 ? <p className="py-5 text-sm text-ink/60">Belum ada top-up untuk anak ini.</p> : null}
            </div>
          </section>

          <section id="celengan" className="panel rounded-lg p-5">
            <h2 className="text-lg font-black">Celengan</h2>
            <form action={createSavingsPocketAction} className="mt-3 flex gap-2">
              <input type="hidden" name="child_id" value={child.id} />
            ...
            <div className="mt-3 divide-y divide-line">
              {data.pockets.map((item) => <div className="py-3 text-sm" key={item.id}>{item.name} - {formatRupiah(Number(item.current_amount))}/{formatRupiah(Number(item.target_amount))} - {item.status}</div>)}
              {data.pockets.length === 0 ? <EmptyState title="Belum ada celengan" description="Buat celengan untuk anak terpilih." /> : null}
            </div>
          </section>
          ...
          <section className="panel rounded-lg p-5">
            ...
            <div className="mt-3 divide-y divide-line">
              ...
              {data.requests.length === 0 ? <EmptyState title="Belum ada saving request" description="Pengajuan pencairan anak terpilih akan tampil di sini." /> : null}
            </div>
          </section>
          ...
          <section className="panel rounded-lg p-5">
            ...
            <div className="mt-3 divide-y divide-line">
              ...
              {data.missions.length === 0 ? <EmptyState title="Belum ada misi" description="Buat misi sederhana untuk anak terpilih." /> : null}
            </div>
          </section>
